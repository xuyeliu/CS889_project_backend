...
class Tokenizer(object):
    ...
    def text_to_seq(self, seq, maxlen=50, padding='post', truncating='post', value=0):
        txt = []
        for w in seq.split():
            try:
                txt.append(self.w2i[w])
            except:
                txt.append(self.oov_index)
            if maxlen is not None:
                if len(txt) == maxlen:
                    break
        return pad_sequences([txt],maxlen=maxlen, padding=padding, truncating=truncating, value=value)
    def forw2v(self, seq):
